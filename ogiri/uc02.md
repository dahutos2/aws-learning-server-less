...
#### 7.4 必要なオブジェクトを tar ファイルの変換

```bash
# 全てのファイルが存在する階層で圧縮を行う
tar -czvf training_code.tar.gz requirements.txt training_script.py inference.py
```

#### 7.5 必要なオブジェクトを S3 にアップロード
...
### 10. SageMaker エンドポイントにオートスケーリング機能を適用する
1. **AWS Management Console**にログインし、**SageMaker**サービスに移動します。
2. 左側のナビゲーションペインで「**エンドポイント**」を選択し、対象のエンドポイントをクリックします。
3. エンドポイントの「**設定**」タブで「**エンドポイントのランタイムの設定**」で、「**自動スケーリングを設定**」をクリックします。
4. 「**バリアントの自動スケーリングの設定**」を以下に設定し、「**保存**」をクリックします。
   - 「**最小インスタンス数**」：`1`
   - 「**最大インスタンス数**」：`10`
   - 「**ターゲット値**」：`15`
   - 「**スケールインのクールダウン**」：`300`
